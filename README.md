...
| 锁屏后解锁 | 成功解锁后重新开始完整的 20 秒，保留自动熄屏模式；输入密码消耗的时间不计入桌面的倒计时。 |
| 开启屏幕 | 双击即可取消自动熄屏和当前倒计时，恢复正常使用。 |
| 轻醒按键 | 执行指定按键的屏幕规则，供键盘、鼠标和外部映射工具调用。 |
| 轻醒设置 | 配置“哪个按键 → 哪个应用 / 项目文件夹”；另提供默认关闭、确认后才启用的 Codex 防亮屏保护。 |
...
新建规则默认停用。普通屏幕唤醒不执行任何目标；只有匹配该规则的按键或映射入口才执行。关闭或删除规则后停止执行，无需修改源码或重新构建。
...
此功能**默认关闭**。拉取代码、构建、复制应用和打开设置，都不会安装或启动后台保护。

需要时打开“轻醒设置 → 防亮屏保护”，勾选“启用 Codex 防亮屏保护”，阅读提示后点击“了解副作用并开启”。确认框默认选择“取消”。自动熄屏期间，也可从菜单栏选择“轻醒设置（退出熄屏模式）…”进入设置。

开启后，在显示器熄屏、会话锁定、自动熄屏模式活动或状态无法确认时，轻醒会暂停当前用户的 Codex 电脑操作服务。正常亮屏、解锁并退出自动熄屏模式后，等待状态稳定再恢复自己暂停的进程。

**副作用：** 桌面操作和屏幕历史记录会暂停，相关请求可能超时；屏幕历史可能因此停止，需要你在 Codex 设置中手动重新开启。轻醒不会替你开启历史记录。此功能不能保证阻止所有来源的亮屏。

取消勾选即可关闭。若服务仍处于暂停状态，轻醒会等电脑恢复正常使用后完成清理，避免在熄屏时突然执行排队中的桌面请求。更多说明、运行环境和移除步骤见 [防亮屏保护](docs/quiet-protection.md)。
