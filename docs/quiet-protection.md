...
Codex 读取桌面、使用桌面历史记录或执行电脑操作时，可能触发亮屏，尤其是涉及实时截图或窗口检查的任务。设置中的“使用说明”展示这一提示。防亮屏保护入口和未部署的 Codex 候选补丁已经移除，轻醒不修改 Codex 安装包。

轻醒的锁屏阶段不再执行倒计时关屏，避免 20 秒期限或旧重试打断密码和指纹验证。正常解锁后重新开始桌面倒计时；指定按键已取消的模式不会恢复。身份验证和锁屏亮起后的超时均交由 macOS 管理。

这项修正不等于全系统的软件唤醒拦截。现有公开接口允许任意应用声明用户活动并点亮显示器，轻醒没有可靠地统一拒绝这些请求的接口。冻结所有相关进程、在密码界面反复关回屏幕，均不符合不干扰解锁的要求。证据、限制和后续实现约束见 [亮屏与解锁排查](wake-and-unlock.md)。
...
旧版开启选择由应用启动时的兼容清理关闭，设置中不再提供保护开关。轻醒不会自动解锁、存储密码或自动开启屏幕历史记录。
...
更新后启动任一轻醒应用会关闭旧版开启选择。排查旧安装时，应检查 `preferences.json` 已关闭及 `runtime/owned.json` 中的 `owned` 数组是否为空；仍有记录时先处理旧服务状态。在记录为空时，可移除已退出的登录任务：
...
`python3 scripts/test.py` 覆盖拒绝重新开启、旧开启配置不能冻结服务、无状态时退出、设置页显示 Codex 亮屏提示且无保护开关，以及旧暂停记录的安全清理。测试只用临时目录、模拟会话及独立 C 测试进程，不操作真实 Codex 服务，不触发锁屏或指纹验证。
